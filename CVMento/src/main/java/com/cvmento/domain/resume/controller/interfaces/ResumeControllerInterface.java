...
public interface ResumeControllerInterface {
    ...
    ResponseEntity<CommonResponse<Page<ResumeStatusListResponse>>> getResumesByStatus(
            @Parameter(description = "페이지 번호 (0부터 시작)", example = "0")
            @RequestParam(defaultValue = "0") int page,
            ...
            @Parameter(description = "작성자 이메일 필터 (부분 검색)", example = "user@example.com")
            @RequestParam(required = false) @Size(max = 320, message = "이메일은 320자를 초과할 수 없습니다.") String email,
            @Parameter(description = "이력서 제목 필터 (부분 검색)", example = "백엔드 개발자")
