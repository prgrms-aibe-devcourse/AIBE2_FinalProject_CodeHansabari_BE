...
public class CoverLetterLlmPromptService {
    ...
    public String buildImprovementPrompt(String content, List<CoverLetterFeatureDto> features) {
        return buildImprovementPrompt(content, features, null);
    }
    ...
    private String buildGuidelines() {
        return """
            ...
            2. **기본 기준 준수**: 우수 자소서 작성 기준을 반드시 반영
            3. **추가 요구사항 반영**: 사용자의 추가 요구사항이 있다면 우선적으로 고려
            4. **개선 원칙**: 
               - 원본이 이미 우수한 품질이라면 과도한 수정을 피하고 필요한 부분만 개선
               - 원본의 핵심 경험과 성과는 유지하되, 표현과 구조를 개선
               - 원본의 내용을 왜곡하거나 삭제하지 않음
               - 좋은 내용은 그대로 유지하고, 부족한 부분만 보완
               - STAR 기법보다는 자연스러운 흐름과 표현에 집중
            5. **구체적 피드백**: description과 suggestion만 간단명료하게 작성
            6. **JSON 형식 준수**: 반드시 유효한 JSON 형식으로 응답
            7. **언어**: 모든 응답은 한국어로 작성
            8. **응답 길이**: 피드백은 간결하게, 개선된 자소서는 2000자 이내로 작성
            """;
    }
}